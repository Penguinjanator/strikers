#!/bin/sh
# Fetch the latest Dawn with the Switch changes into extern/dawn-switch.
set -e
...
# How far below the branch head Aurora's Dawn commit may sit.
DAWN_DEPTH=50

# The branch must build on the Dawn Aurora pins, so the WebGPU headers match Aurora's code.
...
DAWN_REF=$(git ls-remote "$DAWN_REPO" "refs/heads/$DAWN_BRANCH" | cut -f1)
if [ -z "$DAWN_REF" ]; then
    echo "fetch-switch-deps.sh: cannot resolve $DAWN_BRANCH in $DAWN_REPO" >&2
    exit 1
fi
DAWN_STAMP="$DAWN_REF $AURORA_DAWN_REF"

if [ -f "$DAWN_DEST/CMakeLists.txt" ] && [ "$(cat "$DAWN_DEST/.strikers-dawn" 2>/dev/null)" = "$DAWN_STAMP" ]; then
    echo "==> Dawn $DAWN_BRANCH ($DAWN_REF) already in $DAWN_DEST"
    exit 0
fi

echo "==> fetching Dawn $DAWN_BRANCH ($DAWN_REF)"
rm -rf "$DAWN_DEST"
mkdir -p "$DAWN_DEST"
...
if ! git -C "$DAWN_DEST" merge-base --is-ancestor "$AURORA_DAWN_REF" HEAD 2>/dev/null; then
    echo "fetch-switch-deps.sh: $DAWN_BRANCH does not build on Aurora's Dawn $AURORA_DAWN_REF" \
